...
BEGIN
    ...
    BEGIN
        ...
        begin
            ...
            from hotel.dim_room r LEFT JOIN
        ...
        begin
            ...
            UPDATE fact
            SET 
                fact.running_service_cost = fact.running_service_cost + temp.running_service_cost,
                fact.running_service_charge = fact.running_service_charge + temp.running_service_charge,
                fact.running_service_count = fact.running_service_count + temp.running_service_count,
                fact.running_service_discount = fact.running_service_discount + temp.running_service_discount,
                fact.running_room_discount = fact.running_room_discount + s.running_room_discount,
                fact.running_room_charge = fact.running_room_charge + s.running_room_charge,
                fact.avg_duration = CASE 
                    WHEN s.total_stay_time + s.running_number_of_bookings > 0 THEN
                        (fact.avg_duration * fact.running_number_of_bookings + s.total_stay_time) 
                        / (fact.running_number_of_bookings + s.running_number_of_bookings)
                    ELSE 0
                END,
                fact.running_number_of_bookings = fact.running_number_of_bookings + s.running_number_of_bookings
            FROM hotel.fact_acc_hotel fact
            JOIN temp_acc temp 
                ON fact.room_id = temp.room_id AND fact.room_status_id = temp.room_status_id
            JOIN stays s 
...
BEGIN
    ...
    BEGIN
        ...
        begin
            declare  @current_date=='2025-06-11' DATE ; 
            WITH temp_acc AS (
                SELECT 
                    r.room_id,
                    r.status_id AS room_status_id,
